...
void loop(){
  ...
  if(error == 0){
    delay(2000);
    Serial.println(F("Coloque todos os sticks e subtrims na posição central em 10 segundos."));
    for(int i = 9;i > 0;i--){
      delay(1000);
      Serial.print(i);
      Serial.print(" ");
  ...
  if(error == 0){  
    Serial.println(F("Mova o stick do acelerador para aceleração total e de volta ao centro"));
    ...
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Mova o roll stick para simular a asa esquerda para cima e de volta ao centro"));
    //Check for throttle movement
  ...
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Mova o pitch stick para simular o nariz para cima e de volta ao centro"));
    //Check for throttle movement
    check_receiver_inputs(3);
    Serial.print(F("O passo está conectado à entrada digital "));
  ...
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Mova o stick de guinada para simular o nariz para a direita e de volta ao centro"));
    //Check for throttle movement
    check_receiver_inputs(4);
    Serial.print(F("A guinada está conectada à entrada digital "));
  ...
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Mova suavemente todas as varas simultaneamente para suas extensões"));
    Serial.println(F("Quando estiver pronto, coloque os palitos de volta em suas posições centrais"));
    ...
    Serial.println(F("Mova o stick 'nariz para cima' e volte para o centro para continuar"));
    check_to_continue();
  }
  ...
  if(error == 0){
    ...
    Serial.println(F("Procurando MPU-6050 no endereço 0x68/104"));
    delay(1000);
  ...
  if(error == 0){
    ...
    Serial.println(F("==================================================="));
    Serial.println(F("Não mova o quadricóptero!! A calibração começa em 3 segundos"));
    delay(3000);
    Serial.println(F("Calibrando o giroscópio, isso levará +/- 8 segundos"));
    Serial.print(F("Por favor, espere"));
    ...
    Serial.println(F("==================================================="));
    Serial.println(F("Configuração dos eixos giroscópicos"));
    Serial.println(F("==================================================="));
    
    //Detect the left wing up movement
    Serial.println(F("Levante o lado esquerdo do quadricóptero para um ângulo de 45 graus em 10 segundos"));
    //Check axis movement
    check_gyro_axes(1);
    if(error == 0){
      Serial.println(F("OK!"));
      Serial.print(F("Detecção de ângulo ="));
      ...
      Serial.println(F("Coloque o quadcopter de volta em sua posição original"));
      Serial.println(F("Mova o stick 'nariz para cima' e volte para o centro para continuar"));
      check_to_continue();
      ...
      Serial.println(F(""));
      Serial.println(F(""));
      Serial.println(F("Levante o nariz do quadcopter para um ângulo de 45 graus em 10 segundos"));
      //Check axis movement
      check_gyro_axes(2);
    }
    if(error == 0){
      ...
      Serial.println(F(""));
      Serial.println(F(""));
      Serial.println(F("Gire o nariz do quadcopter 45 graus para a direita em 10 segundos"));
      //Check axis movement
      check_gyro_axes(3);
    }
    if(error == 0){
  ...
  if(error == 0){
    ...
    Serial.println(F("O LED deve estar aceso agora"));
    Serial.println(F("Mova o stick 'nariz para cima' e volte para o centro para continuar"));
    check_to_continue();
    digitalWrite(12, LOW);
  }
